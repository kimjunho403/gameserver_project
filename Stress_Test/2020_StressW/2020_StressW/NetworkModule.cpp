...
const static int MAX_TEST = 5000; //클라 max 개수
...
struct CLIENT {
	...
	SOCKET client_socket; //캐릭터 마다 가지는 소켓 
	OverlappedEx recv_over;
	unsigned char packet_buf[MAX_PACKET_SIZE];
	int prev_packet_data;
	int curr_packet_size;
	high_resolution_clock::time_point last_move_time;//클라가 가장 최근에 움직인 시간 
};

array<int, MAX_CLIENTS> client_map;
array<CLIENT, MAX_CLIENTS> g_clients;

atomic_int num_connections;//현재 얼마까지 
atomic_int client_to_close;//렉이 걸리면 접속을 멈춤
//동접이 늘었다고 당장 렉이 걸리는것이 아님 
//먼저 접속해서 잘돌아가고 있는 클라를 끊음 렉이 안걸릴때까지 
...
void ProcessPacket(int ci, unsigned char packet[])
{
		...
		if (move_packet->id < MAX_CLIENTS) {
			...
			if (ci == my_id) {
				if (0 != move_packet->move_time) {
					auto d_ms = duration_cast<milliseconds>(high_resolution_clock::now().time_since_epoch()).count() -move_packet->move_time;//딜레이
	...
	case SC_LOGIN_INFO:
	{
		...
		int my_id = ci;
		client_map[login_packet->id/*서버에서 보내준 id*/] = my_id;
...
void Worker_Thread()
{
	while (true) {
		...
		else if (OP_DO_MOVE == over->event_type) {//안씀
			// Not Implemented Yet
			delete over;
		}
		else {
...
void Adjust_Number_Of_Client()
{
	...
	if (active_clients >= MAX_TEST) return;//동접을 늘일필요가없음
	...
	int t_delay = global_delay;//렉이 심하면 접속을 끊음
	if (DELAY_LIMIT2 < t_delay/*들레이가 크면 접속을 늘이지 않음*/) {
		if (true == increasing) {
...
{
	while (true) {
			Adjust_Number_Of_Client();//동접을 적당한 속도로 늘이고 줄임
			...
			for (int i = 0; i < num_connections; ++i) {
				...
				my_packet.move_time = static_cast<unsigned>(duration_cast<milliseconds>(high_resolution_clock::now().time_since_epoch()).count());//컴퓨터가 세상에 나오고 지난 시간
...
void GetPointCloud(int* size, float** points)
{
	int index = 0;
	for (int i = 0; i < num_connections; ++i)
		if (true == g_clients[i].connected/*x,y좌표를 point cloud에 넣음*/) {
